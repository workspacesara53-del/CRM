...
import { processCampaigns } from "./lib/campaign-processor";
...
async function isSubscriptionActive(ownerId: string): Promise<boolean> {
    try {
        ...
        if (error || !sub) {
            // If No sub record, assume inactive for safety until checked by frontend
            return false;
        }
        ...
        if (sub.status === 'active' && subEnd && now < subEnd) return true;
        if (sub.status === 'trial' && now < trialEnd) return true;

        return false;
    } catch (e) {
        console.error("[SubCheck] Error:", e);
        return false;
    }
}
...
async function downloadAndUploadMedia(msg: any, mediaType: string, sessionId: string, sock?: any): Promise<string | null> {
    try {
        const socket = sock || sessions.get(sessionId);
        const buffer = await downloadMediaMessage(
            ...
            {},
            {
                ...
                reuploadRequest: socket?.updateMediaMessage || (() => Promise.resolve({} as any))
            }
        );
        ...
        const filename = `${sessionId}/${Date.now()}-${Math.random().toString(36).slice(2, 7)}.${ext}`;

        const { data, error } = await supabaseAdmin.storage
            .from("chat-media")
            .upload(filename, buffer, {
                contentType: mediaType === "image" ? "image/jpeg" : undefined,
                cacheControl: "3600",
                upsert: false
            });
        ...
        if (error) {
            ...
            // Fallback: if bucket doesn't exist, log it clearly
            if (error.message?.includes('bucket not found')) {
                console.warn("[Media] Please create a public bucket named 'chat-media' in Supabase Storage.");
            }
            return null;
        }
        ...
        console.log(`[Media] Uploaded ${mediaType} to storage: ${publicUrl}`);
        return publicUrl;
    } catch (error) {
...
async function startSession(sessionId: string) {
    ...
    try {
        ...
        (sock.ev.on as any)("chats.update", async (updates: any[]) => {
            for (const update of updates) {
                if (update.id) {
                    recordChatJid(sessionId, update.id);
                }
            }
        });

        (sock.ev.on as any)("connection.update", async (update: any) => {
            ...
            if (qr) {
                ...
                try {
                    ...
                    else console.log(`?o. QR Code for session ${sessionId} successfully updated`);
                } catch (e) {
        ...
        (sock.ev.on as any)("creds.update", saveCreds);

        // =========================
        // Chats sync (initial + updates)
        // =========================
        (sock.ev.on as any)("chats.set", async ({ chats: waChats }: { chats: any[] }) => {
        ...
        (sock.ev.on as any)("chats.upsert", async (waChats: any[]) => {
            for (const waChat of waChats) {
                try {
                    const jid = waChat.id;
                    if (!jid || jid === "status@broadcast") continue;
        ...
        // History sync events
        // =========================
        (sock.ev.on as any)("messaging-history.set", async ({ chats: historyChats = [], messages: historyMessages = [], isLatest }: { chats: any[], messages: any[], isLatest: boolean }) => {
        ...
        // =========================
        (sock.ev.on as any)("messages.upsert", async ({ messages: newMessages, type }: { messages: any[], type: string }) => {
            console.log("messages.upsert", JSON.stringify({ messages: newMessages, type }, null, 2));
            ...
            if (type === "notify" || type === "append") {
                for (const msg of newMessages) {
                    ...
                    if (msg.message?.imageMessage) {
                        mediaUrl = await downloadAndUploadMedia(msg, "image", sessionId, sock);
                    } else if (msg.message?.videoMessage) {
                        mediaUrl = await downloadAndUploadMedia(msg, "video", sessionId, sock);
                    } else if (msg.message?.audioMessage) {
                        mediaUrl = await downloadAndUploadMedia(msg, "audio", sessionId, sock);
                    } else if (msg.message?.stickerMessage) {
                        mediaUrl = await downloadAndUploadMedia(msg, "sticker", sessionId, sock);
                    } else if (msg.message?.documentMessage) {
                        mediaUrl = await downloadAndUploadMedia(msg, "document", sessionId, sock);
                    }
    ...
    try {
        ...
        if (pendingMessages && pendingMessages.length > 0) {
            for (const messageData of pendingMessages) {
                ...
                if (sendingMessages.has(id)) continue;
...
// Polling for Campaigns (every 10 seconds)
console.log("[Worker] Setting up polling for campaigns (every 10 seconds)...");
setInterval(async () => {
    await processCampaigns(sessions);
}, 10000);
